...
const issuesJson = JSON.stringify(issues);
const issuePrompt = `Given the text: "${text}", identify the issue type from this JSON file: ${issuesJson}. Don't elavorate the response, just return the value from the json file.`;
const severityPrompt = `Given the text: "${text}", identify the severity value from this JSON file: ${issuesJson}. Don't elavorate the response, just return the value from the json file.`;
const priorityPrompt = `Given the text: "${text}", identify the priority value from this JSON file: ${issuesJson}. Don't elavorate the response, just return the value from the json file.`;
  ...
  messages: [
    {
      role: "system",
      content:
        "You are a helpful assistant specialized in customer care processes and you are very concise in your responses just answering with the value from the severity key of the given json file.",
    },
    {
      role: "user",
...
client
  .post("https://api.openai.com/v1/chat/completions", sevParams)
  .then((result) => {
    const severity = result.data.choices[0].message.content;
  ...
  messages: [
    {
      role: "system",
      content:
        "You are a helpful assistant specialized in customer care processes and you are very concise in your responses just answering High, Medium or Low depending of the given json file.",
    },
    {
      role: "user",
...
client
  .post("https://api.openai.com/v1/chat/completions", prioParams)
  .then((result) => {
    const priority = result.data.choices[0].message.content;
